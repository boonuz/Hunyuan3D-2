...
def gen_save_folder(max_size=60):
    ...
    cur_id = min(set(range(max_size)) -
                 exists) if len(exists) < max_size else -1
...
def generation_all(
    ...
    model_viewer_html = build_model_viewer_html(
        save_folder, height=596, width=700)

    textured_mesh = texgen_worker(mesh, image)
    path_textured = export_mesh(textured_mesh, save_folder, textured=True)
    model_viewer_html_textured = build_model_viewer_html(
        save_folder, height=596, width=700, textured=True)
...
def build_app():
    ...
    with gr.Blocks(theme=gr.themes.Base(), title='Hunyuan-3D-2.0') as demo:
        ...
        with gr.Row():
            with gr.Column(scale=2):
                ...
                with gr.Accordion('Advanced Options', open=False):
                    num_steps = gr.Slider(
                        maximum=50, minimum=20, value=30, step=1, label='Inference Steps')
                    octree_resolution = gr.Dropdown(
                        [256, 384, 512], value=256, label='Octree Resolution')
                    cfg_scale = gr.Number(value=5.5, label='Guidance Scale')
                    seed = gr.Slider(maximum=1e7, minimum=0,
                                     value=1234, label='Seed')

                with gr.Group():
                    btn = gr.Button(value='Generate Shape Only',
                                    variant='primary')
                    btn_all = gr.Button(
                        value='Generate Shape and Texture', variant='primary', visible=HAS_TEXTUREGEN)
        ...
        tab_gi.select(fn=lambda: gr.update(
            selected='tab_img_prompt'), outputs=tabs_prompt)
        if HAS_T2I:
            tab_gt.select(fn=lambda: gr.update(
                selected='tab_txt_prompt'), outputs=tabs_prompt)
...
async def generate(request: Request):
    ...
    path, path_textured = generation_all('', image)
...
if __name__ == '__main__':
    ...
    offload.profile(pipe, profile_no=profile,
                    verboseLevel=int(args.verbose), **kwargs)
